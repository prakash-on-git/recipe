...
const AddRecipe = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://localhost:8080/recipe/create", { ...formData, image: imageUrl }, {
    ...
    <div>
      ...
      <div className="container">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex">
            {['prepTime', 'cookTime', 'totalTime', 'servings'].map((field) => (
              <input
                key={field}
                type="number"
                name={field}
                placeholder={`${field.replace(/([A-Z])/g, ' $1')} (mins)`}
          ...
          <div className="input-group">
            <input type="text" value={ingredientInput} placeholder="Add Ingredient" onChange={(e) => setIngredientInput(e.target.value)} />
            <button type="button" onClick={() => addItem(ingredientInput, 'ingredients')}>+</button>
          </div>
          <ul>{formData.ingredients.map((ing, i) => <li key={i}>{ing}</li>)}</ul>

          <div className="input-group">
            <input type="text" value={stepInput} placeholder="Add Step" onChange={(e) => setStepInput(e.target.value)} />
            <button type="button" onClick={() => addItem(stepInput, 'steps')}>+</button>
          </div>
          <ol>{formData.steps.map((step, i) => <li key={i}>{step}</li>)}</ol>
